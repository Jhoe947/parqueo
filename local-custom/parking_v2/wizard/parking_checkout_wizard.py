...
class ParkingCheckoutWizard(models.TransientModel):
    ...
    ticket_id = fields.Many2one('parking2.ticket', string='Ticket', required=True, readonly=True)
    ...
    # amount_total viene del ticket en tiempo real via related
    ...
    amount_paid = fields.Float(string='Monto Recibido del Cliente', digits=(10, 2))
    ...
    def default_get(self, fields_list):
        """Pre-llena amount_paid con el total actual del ticket"""
    ...
    def _compute_change(self):
        for wiz in self:
            wiz.amount_change = max(0.0, (wiz.amount_paid or 0.0) - (wiz.amount_total or 0.0))
    ...
    def _onchange_amount_paid(self):
        if self.amount_paid and self.amount_paid < self.amount_total:
            return {'warning': {
                ...
                'message': 'El monto (%.2f) es menor al total (%.2f).' % (
    ...
    def action_confirm_checkout(self):
        ...
        if paid < total:
            raise ValidationError(
                'El monto recibido (%.2f) es menor al total (%.2f).' % (paid, total)
            )
